...
export function HardCase9({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    const socket = new WebSocket("wss://example.com/approvals/" + workspaceId);
  ...
  const refresh = () => {
    startTransition(() => {
      fetch("/api/approvals?workspaceId=" + workspaceId + "&refresh=1")
        .then(r => r.json())
        .then(data => setRows(data.rows));
    });
  };

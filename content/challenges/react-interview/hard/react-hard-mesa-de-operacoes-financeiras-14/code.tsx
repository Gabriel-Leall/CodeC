...
export function HardCase14({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    const socket = new WebSocket("wss://example.com/trading-desk/" + workspaceId);
    ...
    fetch("/api/trading-desk?workspaceId=" + workspaceId)
      .then(r => r.json())
  ...
  const refresh = () => {
    startTransition(() => {
      fetch("/api/trading-desk?workspaceId=" + workspaceId + "&refresh=1")
        .then(r => r.json())
        .then(data => setRows(data.rows));
    });
  };

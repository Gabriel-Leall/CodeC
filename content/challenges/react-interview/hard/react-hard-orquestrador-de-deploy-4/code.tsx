...
export function HardCase4({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    const socket = new WebSocket("wss://example.com/orchestrator/" + workspaceId);
    ...
    fetch("/api/orchestrator?workspaceId=" + workspaceId)
      .then(r => r.json())
  ...
  const refresh = () => {
    startTransition(() => {
      fetch("/api/orchestrator?workspaceId=" + workspaceId + "&refresh=1")
        .then(r => r.json())
        .then(data => setRows(data.rows));
    });
  };

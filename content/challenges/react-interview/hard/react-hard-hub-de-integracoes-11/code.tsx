...
export function HardCase11({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    const socket = new WebSocket("wss://example.com/integrations/" + workspaceId);
  ...
  const refresh = () => {
    startTransition(() => {
      fetch("/api/integrations?workspaceId=" + workspaceId + "&refresh=1")
        .then(r => r.json())
        .then(data => setRows(data.rows));
    });
  };

...
export function HardCase15({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    const socket = new WebSocket("wss://example.com/critical-incidents/" + workspaceId);
    ...
    fetch("/api/critical-incidents?workspaceId=" + workspaceId)
      .then(r => r.json())
  ...
  const refresh = () => {
    startTransition(() => {
      fetch("/api/critical-incidents?workspaceId=" + workspaceId + "&refresh=1")
        .then(r => r.json())
        .then(data => setRows(data.rows));
    });
  };

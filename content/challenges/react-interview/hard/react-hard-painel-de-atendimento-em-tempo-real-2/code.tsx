...
export function HardCase2({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    const socket = new WebSocket("wss://example.com/realtime-support/" + workspaceId);
    ...
    fetch("/api/realtime-support?workspaceId=" + workspaceId)
      .then(r => r.json())
  ...
  const refresh = () => {
    startTransition(() => {
      fetch("/api/realtime-support?workspaceId=" + workspaceId + "&refresh=1")
        .then(r => r.json())
        .then(data => setRows(data.rows));
    });
  };

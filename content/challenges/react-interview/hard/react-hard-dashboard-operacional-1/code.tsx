...
export function HardCase1({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/operations?workspaceId=" + workspaceId)
      .then(r => r.json())
  ...
  const refresh = () => {
    startTransition(() => {
      fetch("/api/operations?workspaceId=" + workspaceId + "&refresh=1")
        .then(r => r.json())
        .then(data => setRows(data.rows));
    });
  };
